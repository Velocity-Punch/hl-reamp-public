...
#ifdef HLR
enum DrawPolygonMethod_t
{
	DRAWPOLYGON_CENTER = 0,
	DRAWPOLYGON_TOPLEFT,
	DRAWPOLYGON_TOPRIGHT,
	DRAWPOLYGON_BOTTOMRIGHT,
	DRAWPOLYGON_BOTTOMLEFT,
};

// used by DrawPolygon to determine how the 'flPercentage' value should be used. Originally, all images filled from bottom --> top, based on the flpercentage
// Now we have a more flexible method to handle horizontal filling of images.
enum FillMethod_t
{
	FILLMETHOD_BOTTOMTOTOP = 0,
	FILLMETHOD_TOPTOBOTTOM,
	FILLMETHOD_LEFTTORIGHT,
	FILLMETHOD_RIGHTTOLEFT,
};

enum ImageMirrorType_t
{
	MIRROR_NONE = 0,
	MIRROR_LATERAL,
	MIRROR_VERTICAL,
...
public:
	...
	bool DrawPolygon(const char* tex, int x, int y, int iWidth, int iHeight, int iAlpha, float fRadians, int iRotation, ImageMirrorType_t iMirrorType, float flPercent, int r, int g, int b, DrawPolygonMethod_t iDrawMethod, FillMethod_t iFillMethod);
#endif
