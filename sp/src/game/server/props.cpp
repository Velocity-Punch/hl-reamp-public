...
#ifdef HLR
DEFINE_KEYFIELD(m_iSpawnHP,FIELD_INTEGER,"spawnhp"),
#endif
...
DEFINE_OUTPUT(m_pOutputOnDamaged,"OnDamaged"),
DEFINE_OUTPUT(m_pOutputOnKilled, "OnKilled"),
...
#ifdef HLR
	if (HasSpawnFlags(SF_DYNAMICPROP_DAMAGABLE))
	{
		m_takedamage = DAMAGE_YES;
		SetHealth(m_iSpawnHP);
	}
#endif
...
{
	//BaseClass::Event_Killed(info);
	m_pOutputOnKilled.FireOutput(this, NULL);
	m_takedamage = DAMAGE_NO;
}
int CDynamicProp::OnTakeDamage(const CTakeDamageInfo& info)
{
	int ret = BaseClass::OnTakeDamage(info);
	m_pOutputOnDamaged.FireOutput(info.GetAttacker(), this);
	Vector vecDir = info.GetDamageForce().Normalized();
	UTIL_BloodSpray(info.GetDamagePosition(), vecDir, BLOOD_COLOR_RED, 8, FX_BLOODSPRAY_ALL);
	return ret;
}
