...
void CBaseNPCMaker::ChildPostSpawn(CAI_BaseNPC *pChild)
{
	...
	if (m_hEnemyCounter)
	{
		...
		if (counter)
			counter->vStoredNPCs.AddToTail(pChild->entindex());
	}
	pChild->SetModelScale(0.1f);
...
void CBaseNPCMaker::DeathNotice(CBaseEntity *pVictim)
{
	if (m_hEnemyCounter)
	{
		...
		if (counter)
		{
			...
			counter->vStoredNPCs.FindAndFastRemove(pVictim->entindex());
		}
	}
...
void CEnemyCounter::CounterThink()
{
	...
	if (m_iEnemyCount <= m_iDisplayThreshold)
	{
		ShowEnemyPositions();
		if (!bFiredOutput)
		{
...
void CEnemyCounter::ShowEnemyPositions()
{
	for (int i = 0; i < vStoredNPCs.Count(); i++)
	{
		CBaseEntity* pEnt = NULL;

		pEnt = UTIL_EntityByIndex(vStoredNPCs.Element(i));

		if (pEnt != nullptr && pEnt->IsNPC())
		{
			CAI_BaseNPC* npc = dynamic_cast<CAI_BaseNPC*>(pEnt);
			npc->ShowPosition();
		}
	}
}
