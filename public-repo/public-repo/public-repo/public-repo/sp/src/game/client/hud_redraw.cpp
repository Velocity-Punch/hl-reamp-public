...
void CHud::DrawCircularProgressBar(int x, int y, int innerRadius, int outerRadius, int steps, float percentage, Color& clr)
{
	int s_nWhiteTexture = -1;
	...
	float endAngle = startAngle + (M_PI * 2.0f * percentage);

	for (int i = 0; i < steps; i++)
	{
		float frac0 = (float)i / (float)steps;
		float frac1 = ((float)i + 1) / (float)steps;

		float a0 = startAngle + (endAngle - startAngle) * frac0;
		float a1 = startAngle + (endAngle - startAngle) * frac1;

		Vector2D inner0(x + innerRadius * cos(a0), y + innerRadius * sin(a0));
		Vector2D outer0(x + outerRadius * cos(a0), y + outerRadius * sin(a0));
		Vector2D inner1(x + innerRadius * cos(a1), y + innerRadius * sin(a1));
		Vector2D outer1(x + outerRadius * cos(a1), y + outerRadius * sin(a1));
		...
		surface()->DrawTexturedPolygon(3, tri0, false);
		surface()->DrawTexturedPolygon(3, tri1, false);
	}
