...
void CNPC_Antlion::StartTask( const Task_t *pTask )
{
	...
	{
		if (FindWallPoint(GetAbsOrigin(), pTask->flTaskData))
			TaskComplete();
		else
			TaskFail(FAIL_NO_REACHABLE_NODE);
		break;
...
bool CNPC_Antlion::FindWallPoint(const Vector& origin, float distance)
{
	CHintCriteria hintCriteria;

	hintCriteria.SetHintType(HINT_WALL_ATTACK_POINT);
	hintCriteria.SetFlag(bits_HINT_HAS_LOS_TO_PLAYER);
	
	CAI_Hint* pHint = CAI_HintManager::FindHint(this, hintCriteria);

	if (pHint == NULL)
	{
		DevMsg("Antlion couldn't find wall position...\n");
		return false;
	}
	...
	GetHintNode()->GetPosition(this, &vPos);

	AI_NavGoal_t goal(vPos);
