...
void CAI_BaseNPC::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if (m_pPositionSprite != nullptr)
	{
		m_pPositionSprite->SetTransparency(kRenderNone, 0, 0, 0, 0, kRenderFxNone);
		m_pPositionSprite->Remove();
	}
...
#ifdef HLR
bool CAI_BaseNPC::DrawPositionSprite()
{
	if (m_bShouldShowPosition)
	{
		if (!m_pPositionSprite)
		{
			m_pPositionSprite = CSprite::SpriteCreate("sprites/triangle.vmt", GetAbsOrigin() + Vector(0, 0, GetHullHeight() * 1.25f), false);
		}

		m_pPositionSprite->SetTransparency(kRenderTransAdd, 255, 0, 0, 200, kRenderFxNone);
		m_pPositionSprite->SetParent(this);
		m_pPositionSprite->SetScale(0.25f);
...
void CAI_BaseNPC::ShowPosition()
{
	if(!m_bShouldShowPosition)
	{
		m_bShouldShowPosition = true;
		DrawPositionSprite();
	}
}
...
ConVar g_test_threaded_ai("g_test_threaded_ai", "0");
...
//								m_poseMove_Yaw (not saved; recomputed on restore)
	...
	DEFINE_FIELD(m_bShouldShowPosition,FIELD_BOOLEAN),
	DEFINE_FIELD(m_pPositionSprite,FIELD_EHANDLE),
...
void CAI_BaseNPC::Precache( void )
{
	...
	PrecacheMaterial("sprites/triangle.vmt");
