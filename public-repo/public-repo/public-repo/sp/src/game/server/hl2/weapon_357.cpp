...
public:
	...
	void	NormalShot(Vector origin, Vector aimdir);
	void	HeavyShot(Vector origin, Vector aimdir);
...
void CWeapon357::PrimaryAttack(void)
{
	...
	else
	{
		...
		if (GetChargeState() != CHARGESTATE_CHARGED)
		{
			...
			NormalShot(vecSrc, vecAiming);
		}
		else
		{
			DrawLargeBeam();
			HeavyShot(vecSrc, vecAiming);
		}
...
void CWeapon357::NormalShot(Vector origin, Vector aimdir)
{
...
void CWeapon357::HeavyShot(Vector origin, Vector aimdir)
{
	trace_t tr;
	UTIL_TraceLine(origin, origin + (aimdir * MAX_TRACE_LENGTH), MASK_SHOT, GetOwner(), COLLISION_GROUP_NONE, &tr);
	DebugDrawLine(origin, tr.endpos, 255, 0, 0, false, 2.0f);
	Vector mins, maxs;
	...
	{
		trace_t tr2;
		UTIL_TraceHull(origin, origin + (aimdir * MAX_TRACE_LENGTH), mins, maxs, MASK_SHOT, GetOwner(), COLLISION_GROUP_NONE, &tr2);
		DebugDrawLine(tr2.startpos, tr2.endpos, 0, 255, 0, false, 2.0f);
		while(tr2.DidHitNonWorldEntity())
		{
			if (tr2.m_pEnt->IsNPC())
			{
				NormalShot(tr2.endpos, (tr2.m_pEnt->WorldSpaceCenter() - tr2.endpos).Normalized());
			}
			UTIL_TraceHull(tr2.endpos, tr2.endpos + (aimdir * MAX_TRACE_LENGTH), mins, maxs, MASK_SHOT, tr2.m_pEnt, COLLISION_GROUP_NONE, &tr2);
			DebugDrawLine(tr2.startpos, tr2.endpos, 0, 255, 0, false, 2.0f);
		}
	}
	else
	{
		NormalShot(origin, aimdir);
	}
