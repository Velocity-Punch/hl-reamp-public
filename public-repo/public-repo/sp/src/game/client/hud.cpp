...
void RotateVertexRad(int* x, int* y, float flRadians)
{
	int x1, y1;
...
void RotateVertexAng(int* x, int* y, int iAngle)
{
	float flRadians = DEG2RAD(iAngle);
	int x1, y1;
...
bool CHudElement::DrawPolygon(const char* tex, int x, int y, int iWidth, int iHeight, int iAlpha, float fRadians, int iRotation, ImageMirrorType_t iMirrorType, float flPercent, int r, int g, int b, DrawPolygonMethod_t iDrawMethod, FillMethod_t iFillMethod)
{

	int nIcon = vgui::surface()->DrawGetTextureId(tex);
	
	if (nIcon < 0)
	{
		DevMsg("no texture id found for the spinny thing\n");
		nIcon = vgui::surface()->CreateNewTextureID();
		DevMsg("setting id to %i\n", nIcon);
		vgui::surface()->DrawSetTextureFile(nIcon, tex, false, true);
	}
	if (nIcon > 0)
	{
		int vertex1[2], vertex2[2], vertex3[2], vertex4[2];

		//float flPercentDiff = (flPercent - 0.5f) * 2;

		if (iFillMethod == FILLMETHOD_TOPTOBOTTOM)
		{
			vertex1[0] = -1 * iWidth;
			vertex1[1] = iHeight;
			vertex2[0] = iWidth;
		...
		else if (iFillMethod == FILLMETHOD_LEFTTORIGHT)
		{
			vertex1[0] = -1 * iWidth;
			vertex1[1] = iHeight;
			vertex2[0] = iWidth;
		...
		else if (iFillMethod == FILLMETHOD_RIGHTTOLEFT)
		{
			vertex1[0] = -1 * iWidth;
			vertex1[1] = iHeight;
			vertex2[0] = iWidth;
		...
		else // FILLMETHOD_BOTTOMTOTOP (the original method)
		{
			vertex1[0] = -1 * iWidth;
			vertex1[1] = iHeight;
			vertex2[0] = iWidth;
		...
		if (iRotation)
		{
			RotateVertexAng(&vertex1[0], &vertex1[1], iRotation);
			RotateVertexAng(&vertex2[0], &vertex2[1], iRotation);
			RotateVertexAng(&vertex3[0], &vertex3[1], iRotation);
			RotateVertexAng(&vertex4[0], &vertex4[1], iRotation);
		}

		if (fRadians)
		{
			RotateVertexRad(&vertex1[0], &vertex1[1], fRadians);
			RotateVertexRad(&vertex2[0], &vertex2[1], fRadians);
			RotateVertexRad(&vertex3[0], &vertex3[1], fRadians);
			RotateVertexRad(&vertex4[0], &vertex4[1], fRadians);
		}

		int iOffsetX, iOffsetY;
		if (iDrawMethod == DRAWPOLYGON_TOPLEFT)
		{
			iOffsetX = iWidth;
			iOffsetY = iHeight;
		}
		else if (iDrawMethod == DRAWPOLYGON_TOPRIGHT)
		{
			iOffsetX = -1 * iWidth;
			iOffsetY = iHeight;
		}
		else if (iDrawMethod == DRAWPOLYGON_BOTTOMLEFT)
		{
			iOffsetX = iWidth;
			iOffsetY = -1 * iHeight;
		}
		else if (iDrawMethod == DRAWPOLYGON_BOTTOMRIGHT)
		{
			iOffsetX = -1 * iWidth;
			iOffsetY = -1 * iHeight;
		}
		else // if ( iDrawMethod == DRAWPOLYGON_CENTER )
		{
			iOffsetX = 0;
			iOffsetY = 0;
		}
		...
		vertex1[0] += x + iOffsetX;
		vertex1[1] += y + iOffsetY;
		vertex2[0] += x + iOffsetX;
		vertex2[1] += y + iOffsetY;
		vertex3[0] += x + iOffsetX;
		vertex3[1] += y + iOffsetY;
		vertex4[0] += x + iOffsetX;
		vertex4[1] += y + iOffsetY;

		float uv1, uv2, uv1a, uv2a;
		...
		else // if ( iFillMethod == FILLMETHOD_BOTTOMTOTOP )
		{
			uv1 = 0;
			uv1a = 1;
			uv2 = uv2a = 1;
		...
		Vector2D uv11(uv1, uv1a);
		Vector2D uv12(uv1, uv2a);
		Vector2D uv21(uv2, uv1a);
		Vector2D uv22(uv2, uv2a);

		// Draw the main icon
		vgui::Vertex_t vert[4];
		//vgui::surface()->DrawSetColor(r, g, b, iAlpha);
		vgui::surface()->DrawSetTexture(nIcon);
		//vgui::surface()->DrawSetTextureFile(nIcon, texture, false, true);
		if (iMirrorType == MIRROR_LATERAL)
		{
			vert[0].Init(Vector2D(vertex4[0], vertex4[1]), uv21);
			vert[1].Init(Vector2D(vertex3[0], vertex3[1]), uv11);
			vert[2].Init(Vector2D(vertex2[0], vertex2[1]), uv12);
			vert[3].Init(Vector2D(vertex1[0], vertex1[1]), uv22);
		}
		else if (iMirrorType == MIRROR_VERTICAL)
		{
			vert[0].Init(Vector2D(vertex4[0], vertex4[1]), uv12);
			vert[1].Init(Vector2D(vertex3[0], vertex3[1]), uv22);
			vert[2].Init(Vector2D(vertex2[0], vertex2[1]), uv21);
			vert[3].Init(Vector2D(vertex1[0], vertex1[1]), uv11);
		}
		else // no mirroring
		{
...
ConVar hud_dynamic_movement("hud_dynamic_movement", "0", FCVAR_CLIENTDLL | FCVAR_ARCHIVE);
...
#ifdef HLR
// offsets hud element pos based on player lookdir
// x returns as x offset (add to initialpos)
// y returns as y offset (subtract from initialpos)
void DynamicHud(int& x, int& y)
{
	C_BasePlayer* pPlayer = C_BasePlayer::GetLocalPlayer();
	if (!pPlayer)
		return;
	...
	VectorMA(pPlayer->EyePosition(), 256.f, vecEyeDirection, vecEndPos);
	Vector screen;

	crossx = ScreenWidth() / 2;
	crossy = ScreenHeight() / 2;

	HudTransform(vecEndPos, screen);
