...
#define PLASMA_MODEL "models/weapons/projectiles/plasmabeam.mdl"
...
ConVar sk_plasmarifle_plasma_speed("sk_plasmarifle_plasma_speed", "8000");
...
bool CPlasmaBall::DrawSprite(void)
{
	...
	if (m_pMainGlow != NULL)
	{
		...
		if (pPlayer->HasOverdrive())
			m_pMainGlow->SetTransparency(kRenderGlow, 255, 0, 0, 75, kRenderFxNoDissipation);
		else
			m_pMainGlow->SetTransparency(kRenderGlow, 255, 255, 255, 75, kRenderFxNoDissipation);
		m_pMainGlow->SetScale(0.11f);
...
void CWeaponPlasmaRifle::PrimaryAttack(void)
{
	...
	while (m_flNextPrimaryAttack <= gpGlobals->curtime) //while we're allowed to shoot
	{
		...
		pBall->SetTargetPos(tr.endpos, sk_plasmarifle_plasma_speed.GetFloat());
	...
	pPlayer->ViewPunch(QAngle(random->RandomFloat(-0.5f, 0.5f), random->RandomFloat(-0.5f, 0.5f), 0));
