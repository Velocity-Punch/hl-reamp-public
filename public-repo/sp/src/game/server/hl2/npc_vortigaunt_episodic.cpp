...
CAI_BaseNPC* CNPC_Vortigaunt::FindNearestNPCToBuff()
{
	...
	for (CEntitySphereQuery query(WorldSpaceCenter(), sk_vortigaunt_heal_range.GetFloat(), FL_NPC); (pSearch = query.GetCurrentEntity()) != nullptr; query.NextEntity())
	{
		...
		{
			Warning("Buffer: NPC WITH INDEX %i IS CURRENTLY IN THE EFFECT LIST, VERIFYING\n", pNPC->entindex());

			if (!VerifyNPC(pNPC))
			{
				Warning("Buffer: NPC CURRENTLY HAS NO EFFECTS, REMOVING FROM AFFECTED NPC LIST\n");
				g_pGameRules->g_utlvec_vorteffectlist.FindAndFastRemove(pNPC->entindex());
				continue;
			}
			else
			{
				Warning("Buffer: NPC IS CURRENTLY BEING AFFECTED, IGNORING IT\n");
				continue;
			}
		}
...
CAI_BaseNPC* CNPC_Vortigaunt::FindNearestNPCToShield()
{
	...
	for (CEntitySphereQuery query(WorldSpaceCenter(), sk_vortigaunt_heal_range.GetFloat(), FL_NPC); (pSearch = query.GetCurrentEntity()) != nullptr; query.NextEntity()) //iterate through all npcs in radius
	{
		...
		if (g_pGameRules->g_utlvec_vorteffectlist.HasElement(pNPC->entindex()))
		{
			Warning("Shielder: NPC %s WITH INDEX %i IS CURRENTLY IN THE EFFECT LIST, VERIFYING\n", pNPC->GetClassname(),pNPC->entindex());
			if (!VerifyNPC(pNPC))
			{
	...
	{
		Warning("Shielder found no candidates\n");
		return NULL;
	}
