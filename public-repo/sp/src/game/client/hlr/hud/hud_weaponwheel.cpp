...
private:
	...
	int iSelectedWeaponNumber;
	float radPointerAngle;
...
CHudWeaponWheel::CHudWeaponWheel(const char* pElementName) : CHudElement(pElementName), BaseClass(NULL, "HudWeaponWheel")
{
	...
	angleTextureid = surface()->CreateNewTextureID();
...
float LerpAngle(float current, float dest, float t)
{
	return current + AngleDiff(dest, current) * t;
}
void CHudWeaponWheel::Paint()
{
	...
	int iCenterRadius = GetWide() * 0.5;

	Vector2D vecCenter, vecCursor;
	vecCenter.Init(screenCenterX, screenCenterY);
	vecCursor.Init(endposx, endposy);

	vecCursor = Vector2D(endposx, endposy);

	//Vector2D vecRad = vecCursor - vecCenter;

	//float rads = atan2(vecRad.y, vecRad.x);

	float destAngle = (360.f / MAX_WHEEL_SLOTS) * iSelectedWeaponNumber;
	 
	radPointerAngle = LerpAngle(radPointerAngle, destAngle, 0.15f);

	float intang = roundf(radPointerAngle);

	//engine->Con_NPrintf(0, "destangle:%f\nlerpangle:%f\nintang:%f", destAngle, radPointerAngle, intang);

	DrawPolygon("HUD/weaponwheel_pointer", screenCenterX, screenCenterY, iCenterRadius, iCenterRadius, 200, 0, intang, MIRROR_NONE, 1.f, 200, 200, 200, DRAWPOLYGON_CENTER, FILLMETHOD_TOPTOBOTTOM);
#endif
