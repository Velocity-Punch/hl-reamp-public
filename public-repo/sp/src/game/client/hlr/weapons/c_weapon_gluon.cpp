...
ConVar g_gluon_pitchup_time("g_gluon_pitchup_time", "15", FCVAR_CLIENTDLL);
...
void C_WeaponGluon::StartFX()
{
	...
	if (!m_sBaseLayer)
	{
		...
		CSoundEnvelopeController::GetController().SoundChangePitch(m_sBaseLayer, 150, g_gluon_pitchup_time.GetFloat());
	}
	if (!m_sMidLayer)
	{
		...
		CSoundEnvelopeController::GetController().SoundChangePitch(m_sMidLayer, 150, g_gluon_pitchup_time.GetFloat());
	}
