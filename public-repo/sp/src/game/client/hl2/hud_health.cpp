...
void CHudHealth::ApplySettings(KeyValues* inResourceData)
{
...
void CHudHealth::Paint(void)
{
	...
	ConVarRef hud_dynamic_movement("hud_dynamic_movement");
	if (hud_dynamic_movement.GetBool())
	{
		int offsetx, offsety;
		DynamicHud(offsetx, offsety);
		engine->Con_NPrintf(0, "orig x: %i orig y: %i offset x %i offset y %i\n", origx, origy, offsetx, offsety);
		SetPos(origx + offsetx, origy - offsety);
	}
}
